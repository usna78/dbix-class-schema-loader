...
This packaged is added to DBIx::Class::Schema::Loader::DBI to reflect the need to use DBD::MariaDB and DBIx::Class::Storage::DBI::MariaDB when using a MariaDB client newer than those supported by the DBD::mysql driver.
...
sub _system_schemas {
    ...
    return ($self->next::method(@_), 'MariaDB');
}
...
# MariaDB does not have a DEFERRABLE attribute, but there is a way to defer FKs.
...
sub _MariaDB_table_get_keys {
    my ($self, $table) = @_;

    if(!exists($self->{_cache}->{_MariaDB_keys}->{$table->sql_name})) {
        my %keydata;
        ...
        $self->{_cache}->{_MariaDB_keys}->{$table->sql_name} = \%keydata;
    }

    return $self->{_cache}->{_MariaDB_keys}->{$table->sql_name};
}

sub _table_pk_info {
    my ( $self, $table ) = @_;

    return $self->_MariaDB_table_get_keys($table)->{PRIMARY};
}
...
sub _table_uniq_info {
    ...
    my @uniqs;
    my $keydata = $self->_MariaDB_table_get_keys($table);
...
#AAA
 ...
 #  if ($dbi_info->{MariaDB_values}) {
    ...
    if ((not blessed $dbi_info) # isa $sth
        ...
        && lc($dbi_info->{MariaDB_type_name}) eq 'timestamp') {
